...
const responder = async (telefone, id, nome, mensagem) => {
    ...
    let infosUsuario = await query(
        `SELECT wa_id, wa_name, last_interaction, current_step, ticketid
    ...
    if (infosUsuario.rowCount === 0) {
        await query(
            `INSERT INTO ticketbotposition (wa_id, wa_name, last_interaction, current_step, ticketid) 
             VALUES ($1, $2, current_timestamp, 0, (SELECT "ticketId" FROM "Messages" WHERE id = $3))`,
            [telefone, nome, id]);
    ...
    if (ETAPA_ATUAL === 0 || ETAPA_ATUAL === null || tempoInativo) {
        ...
        if(!checkHorarioDeAtendimento(data))
            ...
            await sendMessage(telefone, opcoesMenu(NOME_DA_FILA_PREF));
        }
        return;
    }
    ...
    // Op inválida menu 1
    if (ETAPA_ATUAL === 1) {
        await sendMessage(telefone, menuSelecionado(getLastNumber(mensagem)));
        await sendMessage(telefone, opcoesMenu(NOME_DA_FILA_PREF));
        return;
    }
