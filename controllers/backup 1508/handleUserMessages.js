...
const handleMenuLimit = async (telefone, currentCount) => {
    ...
    if (newCount >= MAX_ATTEMPTS) {
        ...
        await query(
            `UPDATE ticketbotposition
             SET menu_block_until = NOW() + INTERVAL '30 minutes'
             WHERE wa_id = $1`, [telefone]);
        ...
        await query(
            `UPDATE ticketbotposition
             SET menu_sent_count = $2
             WHERE wa_id = $1`, [telefone, newCount]);
...
const responder = async (telefone, id, nome, mensagem) => {
    ...
    // Op inválida menu 1
    if (ETAPA_ATUAL === 1) {
        const isBlocked = await handleMenuLimit(telefone, menu_sent_count);
        if (!isBlocked) {
